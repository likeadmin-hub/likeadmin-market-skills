...
doc_type: model_api
ai_model_id: 89
model_code: kling-v3-std-silent
...
existing_slug: m-kling-v3-std-silent-klingyc
existing_title: "可灵 V3 标准无声（klingYC）"
---

# 可灵 V3 标准无声

视频生成（标准画质、无声）。支持文生视频、图生视频与首尾帧；时长约 3～15 秒。
...
| 类型 | 模型 API |
| 模型名称 | 可灵 V3 标准无声 |
| 模型编码 | `kling-v3-std-silent` |
| 模型类型 | 视频生成（video） |
| 调用方式 | `POST /api/v1/tasks` |
| 调用模式 | 异步 |
| 计费方式 | 按输入时长（33.500000 元 / 分钟） |
| 最大 token | 1 |
