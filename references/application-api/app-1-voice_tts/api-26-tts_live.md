...
existing_slug: voice-tts-tts-live
existing_title: "语音TTS · 文字转语音（Live·异步）"
---

# 语音TTS · 文字转语音（Live·异步）

WebSocket 流式服务，长文本异步合成；返回 task_id，由 fish_tts:worker 执行；服务端可通过多槽位 Worker 并发消费任务。
...
```bash
php think fish_tts:worker
```

并发消费时启动多个分槽 Worker，`worker-id` 从 1 到 `workers`：

```bash
php think fish_tts:worker --workers=3 --worker-id=1
php think fish_tts:worker --workers=3 --worker-id=2
php think fish_tts:worker --workers=3 --worker-id=3
```
