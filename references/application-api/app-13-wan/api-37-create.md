...
existing_doc_id: 97
existing_slug: wan-create
existing_title: "Wan 视频生成 · 创建任务"
---

# Wan 视频生成 · 创建任务

提交 Wan 模型族视频任务。用户只对接平台统一接口，不需要直接请求 APIMart；平台会按 `model` 选择对应能力，并负责参数过滤、任务入库、扣点冻结和轮询转存。
...
| `duration` | integer | 否 | `5` | 以 APIMart 当前模型支持为准 | `5` | 生成视频时长，单位秒；平台按该值乘以单价扣点 |
| `image_with_roles` | array | `wan2.7-r2v` 必填 | - | - | `[{"url":"https://example.com/role.png","role":"first_frame"}]` | 参考图列表；每项至少传 `url`，`role` 可传 `first_frame` / `last_frame` 等 |
| `video_urls` | array | `wan2.7-videoedit` 必填 | - | - | `["https://example.com/input.mp4"]` | 待编辑视频 URL 列表；传入后按 `with_video` 档位计费 |
| `negative_prompt` | string | 否 | - | - | `低清晰度、畸形、闪烁` | 不希望出现在画面中的内容 |
| `seed` | integer | 否 | - | - | `42` | 随机种子，用于尽量复现结果 |
| `watermark` | boolean | 否 | - | `true` / `false` | `false` | 是否添加水印；具体效果以上游支持为准 |
| `callback_url` | string | 否 | - | HTTPS URL | `https://your-domain.com/webhook/wan` | 任务完成或失败时的平台回调地址 |
| `metadata` | object | 否 | - | - | `{"biz_id":"order_1001"}` | 业务透传字段，便于回调或日志关联 |
